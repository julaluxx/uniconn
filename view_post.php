        ...
        <div class="card bg-base-100 shadow-2xl p-6 mb-6">
            <div class="card-body">
                ...
                <div class="flex items-center gap-3 mt-2 text-sm text-gray-500">
                    <img src="<?php echo htmlspecialchars($post['profile_pic'] ?? 'default.jpg'); ?>"
                        class="w-8 h-8 rounded-full border border-primary/30">
                ...
                <p class="mt-4 text-gray-700 whitespace-pre-line leading-relaxed">
        ...
        <dialog id="reportPostModal" class="modal">
            <div class="modal-box">
                ...
                <form method="POST">
                    ...
                    <textarea name="reason" class="textarea textarea-bordered w-full"
                        placeholder="เหตุผลในการรายงาน (ไม่บังคับ)"></textarea>
                ...
                <?php foreach ($comments as $cm): ?>
                    <div class="card bg-base-100 shadow p-4 mb-3">
                        <div class="flex items-center gap-3 mb-2">
                            <img src="<?php echo htmlspecialchars($cm['profile_pic'] ?? 'default.jpg'); ?>"
                                class="w-10 h-10 rounded-full border border-primary/30">
                            <div>
                                <p class="font-bold"><?php echo htmlspecialchars($cm['username']); ?></p>
                                <p class="text-sm text-gray-500"><?php echo date('d/m/Y H:i', strtotime($cm['created_at'])); ?>
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                            <?php if (isset($_SESSION['user_id'])): ?>
                                <button class="btn btn-warning btn-sm"
                                    onclick="showModal('reportComment-<?php echo $cm['id']; ?>')">🚨 รายงาน</button>
                                        ...
                                        <form method="POST">
                                            ...
                                            <div class="modal-action">
                                                ...
                                                <button type="button" class="btn"
                                                    onclick="closeModal('reportComment-<?php echo $cm['id']; ?>')">ยกเลิก</button>
                                            </div>
                                        </form>
                                    </div>
                                </dialog>
            ...
            <section class="mt-8">
                ...
                <form method="POST">
                    <textarea name="content" class="textarea textarea-bordered w-full"
                        placeholder="พิมพ์ความคิดเห็นของคุณ..." required></textarea>
                    <button class="btn btn-primary mt-2">ส่งความคิดเห็น</button>
                </form>
            </section>
        <?php else: ?>
            <p class="text-gray-600 mt-4">กรุณา <a href="login.php" class="link link-primary">เข้าสู่ระบบ</a>
